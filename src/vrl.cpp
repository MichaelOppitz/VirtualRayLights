...
#include "vrl_distanceFunctions.h"
...
int render(RenderSettings settings) {
	...
	for (int i = 0; i < settings.spp; i++) {
		...
		fprintf(stdout, "\rRendering: %f", i / (double)settings.spp * 100);
...
int main(int argc, char *argv[]) {
	...
	for (int i = 1; i < argc; i+=2) {
		...
		if (arg1 == "-h") {
			...
			settings.sampling = max(0, min(4, stoi(arg2)));
		} else if (arg1 == "-b") {
			settings.sampling = max(0, min(INT_MAX, stoi(arg2)));
		} else if (arg1 == "-sa") {
